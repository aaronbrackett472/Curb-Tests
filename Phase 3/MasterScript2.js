...
var curbAddress = "5904 Richmond Hwy, 22303"
var verifiablePhone = "3018328979"
...
//Need a function that will log out, log into the account using the verified phone number
//Log back in to the original account

function freeVerifiedPhone(){
    window.buttons()["Settings"].tap();
    window.tableViews()[0].cells()["Profile"].tap();
    var email = window.tableViews()[0].cells()[1].textFields()[0].value();
    ...
    window.textFields()[0].setValue(verifiablePhone);
    window.secureTextFields()[0].setValue("aaaaaa");
    window.buttons()["Sign In"].tap();
    window.buttons()["Settings"].tap();
    window.tableViews()[0].cells()["Profile"];
    var phoneField = window.tableViews()[0].cells()[2].textFields()[0];
    if (phoneField.value() === verifiablePhone){
        var newPhoneSuffix = randomNumber;
        var newPhone = "555" + randomNumber;
...
function testUserInputsCase1(){
    ...
    closeKeyboard();
    create();
    while (!window.textViews()[0].isValid()){
...
function testUserInputsCase3(){
    ...
    backButton.tap();
    while (window.staticTexts()["Done Editing"].isValid()){
        if (!window.activityIndicators()["In progress"].isValid()){
            if (!window.staticTexts()["Profile"].isValid()){
                backButton.tap()
            }
        }
    }
    while (window.staticTexts()["Account"].isValid()){
        if (!window.activityIndicators()["In progress"].isValid()){
            backButton.tap();
        }
    }
...
function testPreferredProviders(){
	...
	window.buttons()[2].tap();
    target.delay(5);
    while (window.searchBars()[0].value() !== curbAddress){
        window.searchBars()[0].setValue(curbAddress);
    }
	window.buttons()["Search For Location, Double Tap to start searching"].tap();
	window.searchBars()[0].searchBars()[0].tap();
	window.tableViews()[1].tapWithOptions({tapOffset:{x:0.64, y:0.07}});
	...
	while (!window.staticTexts()["Fleet Selection"].isValid()){
        ...
        if (!window.activityIndicators()["In progress"].isValid()){
            ...
            if (!window.staticTexts()["Fleet Selection"].isValid()){
                window.searchBars()[0].searchBars()[0].tap();
    ...
    while (window.staticTexts()["Confirmation"].isValid()){
        //wait for it to confirm your ride
    }
    if (window.staticTexts()["Profile"].isValid()){
        //Verify phone number
        window.tableViews()[0].cells()[2].textFields()[0].setValue("");
        window.tableViews()[0].cells()[2].textFields()[0].tap();
        app.keyboard().typeString(verifiablePhone);//Aaron Brackett's personal number for now
        window.buttons()["Done Editing"].tap();
        //Check to make sure number not already in use
        if (window.textViews()["The phone number you submitted has already been used."].isValid()){
            window.buttons()[3].tap();
            window.buttons()["Back"].tap();
            freeVerifiedPhone();
    ...
    while (window.activityIndicators()["In progress"].isValid()){
...
function testAddCC2(){
    ...
    //If there is a new version, it will tell you now
    UIATarget.onAlert = function onAlert(alert){
        alert.collectionViews()[0].cells()["Ignore"].tap();
        return true;
    }
...
function testPayment(){
    ...
    while (!window.staticTexts()["Fleet Selection"].isValid()){
        ...
        if (!window.activityIndicators()["In progress"].isValid()){
            ...
            if (window.searchBars()[0].searchBars()[0].isValid()){
                window.searchBars()[0].searchBars()[0].tap();
                app.keyboard().typeString("\n");
            }
        }
    ...
    function advanceRide(){
        ...
        target.delay(5);
        if (window.buttons()["Yes"].isValid()){
            window.buttons()["Yes"].tap();
        }
        else{
            window.buttons()["Yea Meter On"].tap();
        }
    }
    ...
    target.delay(5)
    if (window.buttons()["Got it"].isValid()){
...
function testAirports(){
    ...
    while (window.staticTexts()["Pickup Location"].isVisible()){
        //Wait for fleets to be found
        //in case it goes back to previous screen
        var tries = 0;
        if (!window.activityIndicators()["In progress"].isValid()){
            ...
            target.tap({x:192.67, y:361.00});//try tapping pick me up here again
        }
    }
...
function reset(){
    ...
    var phoneField = window.tableViews()[0].cells()[2].textFields()[0];
    if (phoneField.value() === verifiablePhone){
        var newPhoneSuffix = getRandomNumber();
        var newPhone = "555" + newPhoneSuffix;
        phoneField.setValue(newPhone);
        window.buttons()["Done Editing"].tap()
    }
    window.buttons()["Back"].tap();
